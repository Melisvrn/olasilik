...
var histogramOlustur = function(params){
  ...
  var classNumber = Math.round(1 + (3.3* Math.log10(len)));
  var grupGenişliği =Math.round( range / classNumber);

  var labels = new Array(classNumber);
  labels[0] = minValue;

  for(var i = 1; i<= classNumber-1;i++){
  labels[i] = labels[i-1] + grupGenişliği;
  }
  var data = new Array(classNumber);
  for(var j = 0; j < classNumber;j++){
  var sayac = 0;
  for(var k = 0; k<len ;k++){
    if(params[k]>=dizi[j] && params[k] < dizi[j+1]){
    sayac++;
    }
  }
  data[j]=sayac;
